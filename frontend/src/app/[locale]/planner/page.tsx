    ...
    return (
      <div className="min-h-[calc(100vh-64px)] flex flex-col items-center justify-center p-6 text-center">
        <div className="bg-white p-12 rounded-[3rem] shadow-xl max-w-md">
          ...
          <p className="text-slate-500 mb-8">You need to join or create a family group before you can plan meals.</p>
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="max-w-6xl mx-auto px-6 pt-12">
        ...
        {!plan ? (
          <div className="bg-white rounded-[3rem] p-16 text-center border-2 border-dashed border-slate-200 animate-in fade-in zoom-in-95 duration-500 shadow-sm">
            <div className="w-20 h-20 bg-slate-50 rounded-3xl flex items-center justify-center mb-8 mx-auto text-slate-300 border border-slate-100">
              <ChefHat size={40} />
            </div>
            <h2 className="text-2xl font-black text-accent mb-3">{t('no_plan')}</h2>
            <p className="text-slate-500 max-w-sm mx-auto mb-8 font-medium">{t('no_plan_sub')}</p>
          </div>
        ) : (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {days.map((day) => {
                ...
                return (
                  <div key={day.key} className="group bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300">
